...
class GaniToJson {
    ...
    parseLine(line) {
        ...
        switch (lineType) {
            case "SPRITE":
                ...
                this.jsonObject.sprites[sprite.id] = sprite.array.map(a => parseInt(`${a}`));
                break;
            case "SETBACKTO":
            ...
            default:
                if (this.inAnimation && !this.finishedAnimation) {
                    console.log('compileIntoAnimation', line);
    ...
    parseSingleDirectionAnimationFrames() {
        console.log('animation frames was', this.animationFrames);
        const animationFrames = this.animationFrames;
